...
my $netlists_path = "/proj/gateway4/products/gs70/expts/a0393831/SIMULATIONS/netlists";
...
my $file = "codegen_${process}_${temp}_${vdds}_${res}.pl";
open(PERLSCRIPT ,">./$file") || die "Failed to open $file in write mode\n";

print PERLSCRIPT "#!/usr/bin/perl
use strict;

# Script to generate code files
...
my \$cycle=1;
for (\$cycle=1 ; \$cycle < $limcycle ; \$cycle++){
...
#print \"Cycle=\$cycle  IncP = \$incp  IncN = \$incn  Pout = \$pout  Nout = \$nout\\n\\n\";
...
";
close(PERLSCRIPT);
#\!(system("/usr/bin/perl $file")) || die "Code Generation script for $file failed\n";
!(system("/usr/local/lsf/bin/bsub -P P0:IO_10015123 -q short -R ws40 -sp 100 -J $file /usr/bin/perl $file")) || die "LSF submission for $file failed\n";
            }
        }
    }
}
